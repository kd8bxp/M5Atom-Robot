//Gesture Controlled Robot Code - by LeRoy F. Miller, KD8BXP Nov 25, 2023
...
WiFi.mode(WIFI_STA);
  // Init the sensor
   ...
   while (sensor.begin() != 0) {
    ...
    sensor.setGestureHighRate(false); //true);
  log("Begin ok!");
...
void loop()
{
 ...
 /*  Gesture numbers converted to what the ESPNOWBotv1.0.1 sketch      
  *   expects, this saves from having to re-write that sketch
4 up
8 down
1 right
2 left
...
32 backward
  ...
  */
             String description = sensor.gestureDescription(gesture);
              log((String)gesture + " " + description);
              if (gesture == 4) {SendData = 9; } //speed up
              if (gesture == 8) {SendData = 10; } //slow down
              if (gesture == 1) {SendData = 7; } //right
              if (gesture == 2) {SendData = 6; } //left
              if (gesture == 16) {SendData = 22; } //forward
              if (gesture == 32) {SendData = 8; } //reverse
                sendData(SendData);
  }
...
M5.update();
if (M5.BtnA.wasPressed()  ) {sendData(5);} //stop
    else {sendData(0);}
